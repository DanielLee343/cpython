...
#pragma section("PyRuntime", read, write)
    ...
    */
    ...
    pycore_interp_init(PyThreadState *tstate)
    {
        ...
        /* add logic here */
        // status = pycore_init_op_table(interp);
        if (_PyStatus_EXCEPTION(status))
        {
            return status;
        }
...
#undef STORE_MODULE_WEAKREF
    ...
    */
...
#if defined(F_GETFD) && (defined(__linux__) || \
                         defined(__APPLE__) || \
                         defined(__wasm__))
    ...
    _Py_END_SUPPRESS_IPH return (hfile != INVALID_HANDLE_VALUE && GetFileType(hfile) != FILE_TYPE_UNKNOWN);
#else
    struct stat st;
    ...
    _Py_DumpExtensionModules(int fd, PyInterpreterState *interp)
    {
        ...
        if (interp->sysdict != NULL)
        {
            ...
            {
                if (PyUnicode_Check(key) && PyUnicode_CompareWithASCIIString(key, "stdlib_module_names") == 0)
                {
...
#undef Py_FatalError
    ...
    _Py_FdIsInteractive(FILE *fp, PyObject *filename)
    {
        ...
        return ((filename == NULL) || (PyUnicode_CompareWithASCIIString(filename, "<stdin>") == 0) || (PyUnicode_CompareWithASCIIString(filename, "???") == 0));
    }
