...
PyObject_GetIter(PyObject *o)
{
    ...
    /* debugging */
    // if ((void *)t == (void *)0xffffffffffffffff)
    // {
    //     fprintf(stderr, "type not set, wrong~!!!!\n");
    // }
    getiterfunc f;
...
int PyAIter_Check(PyObject *obj)
{
    PyTypeObject *tp = Py_TYPE(obj);
...
PyIter_Next(PyObject *iter)
{
    ...
    {
        PyThreadState *tstate = _PyThreadState_GET();
        if (_PyErr_Occurred(tstate) && _PyErr_ExceptionMatches(tstate, PyExc_StopIteration))
        {
            _PyErr_Clear(tstate);
        }
    }
